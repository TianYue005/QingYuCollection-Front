  ...
  <div class="circle-create-page">
    ...
    <div class="page-header">
      <div class="header-title">
        ...
        <span v-if="!firstLoading" class="page-desc">这里展示你发布过的圈子内容</span>
      </div>
    ...
    <!-- 分类查看：圈子动态 / 跑腿任务 / 热门活动（对应后端 option） -->
...
import { getActivityStatus, getMyCreateCircle, type Circle } from '@/api/forum'
...
/* ---- 分类查看按钮：与后端 option 一一对应 ---- */
type CircleOption = 'Dynamics' | 'Task' | 'Event'
const viewOptions: { key: CircleOption; name: string }[] = [
  { key: 'Dynamics', name: '圈子动态' },
  { key: 'Task', name: '跑腿任务' },
  { key: 'Event', name: '热门活动' },
]
const activeView = ref<CircleOption>('Dynamics')

/** 空状态文案随当前分类变化 */
const emptyTitle = computed(() => {
  if (activeView.value === 'Dynamics') return '还没有发布过动态'
  return activeView.value === 'Task' ? '还没有发布过跑腿任务' : '还没有创建过热门活动'
})
const emptyDesc = computed(() => {
  if (activeView.value === 'Dynamics') return '去圈子板块分享一条动态吧'
  return activeView.value === 'Task' ? '去圈子板块发布一个跑腿任务吧' : '去圈子板块创建一场活动吧'
})
...
/* ---- 列表查询：/forum/myCreate/circle，由 option 区分动态/任务/活动 ---- */
...
async function fetchList(targetPage: number) {
  ...
  error.value = ''
  try {
    // 后端各分支 SQL 均未返回 category，需前端按当前分类补齐，供徽章与详情跳转使用
    const current = activeView.value
    const res = await getMyCreateCircle({
      option: current,
    ...
    if (p) {
      ...
      total.value = p.total
      list.value = p.rows.map((item) => ({ ...item, category: current }))
    } else {
...
/** 切换查看分类：重置分页与列表，重新拉取对应 option 的数据 */
function switchView(key: CircleOption) {
