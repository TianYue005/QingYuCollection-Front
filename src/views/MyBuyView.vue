  ...
  <div class="buy-page">
    ...
    <div class="page-header">
      <div class="header-title">
        <h2 class="page-title">我购买的</h2>
        <span v-if="!firstLoading" class="page-desc">这里展示你已经购买成功的宝贝</span>
      </div>
    ...
    <!-- 空状态 -->
    <div v-else-if="buyList.length === 0" class="state-box">
      <div class="state-icon">
        <svg viewBox="0 0 64 64" fill="none" stroke="currentColor" stroke-width="1.5">
      ...
      <p class="state-title">还没有购买的商品</p>
      <p class="state-desc">当你的交易完成后，这里会展示你买到的宝贝</p>
    </div>

    <!-- 购买记录列表 -->
    <template v-else>
      <div class="buy-list">
        <div v-for="item in buyList" :key="String(item.goodsId)" class="buy-row">
          <!-- 商品图片 -->
          <div class="thumb">
          ...
          <!-- 状态 -->
          <div class="row-side">
            <span class="bought-badge">已购买</span>
            <!-- 无评价时展示“评论卖家”按钮，已评价（hasEvaluate=1）则不显示 -->
...
import { addEvaluate, getMyPurchasedItems, type SoldGoodsVO } from '@/api/item'

/* ---- 列表数据 ---- */
const firstLoading = ref(true)
const loading = ref(false)
const error = ref('')
const buyList = ref<SoldGoodsVO[]>([])
const page = ref(1)
const pageSize = 10
const total = ref(0)
...
/** 该笔交易是否已评价（hasEvaluate=1 为有评价；0/缺失视为未评价，需要展示“评论卖家”） */
...
async function fetchList(targetPage: number) {
  ...
  error.value = ''
  try {
    const res = await getMyPurchasedItems({
